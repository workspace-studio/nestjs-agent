...
- **Propagate union types down** — if a service uses a union type (e.g., `AuditTable`), don't widen it to `string` at the repository boundary. The repository should accept the same union type.
- **Check dead imports after refactoring** — when moving logic between classes, verify the source class still uses its Logger, imports, and injected dependencies. Remove what's orphaned.

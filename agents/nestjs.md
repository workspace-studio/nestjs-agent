...
This is not a reminder. It is a gate. Run it before every `git commit` and print the filled-in checklist. If any line is red, you do not commit.
...
[ ] npm run test             → X passed, Y failed
...
"Those tests were already red" is not a valid excuse to commit. If `npm run test` or `npm run test:e2e` has ANY failures, you MUST do ONE of:
...
Never silently move past a red gate. Never write "pre-existing failure, not related" in a PR description without a linked issue. Every commit must leave the suite the same color or greener, never redder.
